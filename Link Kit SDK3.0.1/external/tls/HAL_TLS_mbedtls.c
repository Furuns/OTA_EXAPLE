...
//can add
#define MY_TLS_PAYLOAD_SIZE(fd)                  getIINCHIP_TxMAX(fd)
...
void *_SSLCalloc_wrapper(size_t n, size_t size)
{
    ...
    if (NULL == buf) {
		printf("(wzfu test printf)malloc size mem failed:%d\r\n",n * size + sizeof(mbedtls_mem_info_t));
        return NULL;
    } else {
		printf("(wzfu test printf)mallocsize:%d,bufaddr:%p\r\n",n * size + sizeof(mbedtls_mem_info_t),buf);
        memset(buf, 0, n * size + sizeof(mbedtls_mem_info_t));
//		if( n * size + sizeof(mbedtls_mem_info_t) > 8000 )
//		{
//			n = n;
//		}
    }
...
//    printf("INFO ++ mbedtls malloc: %p %d  total used: %d  max used: %d  %d*size malloc addr: %p\r\n",
//                       buf, (int)mem_info->size, mbedtls_mem_used, mbedtls_max_mem_used, n, buf);
...
uintptr_t my_HAL_SSL_Establish(const char *host,
                            uint16_t port,
                            const char *ca_crt,
                            uint32_t ca_crt_len)
{
	...
	printf("(wzfu test printf)pTlsDataMallocSize:%d,pTlsDataAddr:%p\r\n", sizeof(TLSDataParams_t),pTlsData);
...
//=========================================================================================================
//can add
MY_TLS_Status_t ReceiveData(uint8_t socket, uint8_t *pdata, uint16_t Reqlen, uint16_t* RcvDatalen, uint32_t Timeout)
{
	MY_TLS_Status_t ret = MY_TLS_STATUS_ERROR; 
	uint16_t rx_len = 0;
	...
	do{
		...
		{
			setSn_IR(socket, Sn_IR_CON);				// Sn_IR的CON位置1，通知W5500连接已建立
		}
		...
		if(rx_len >0)
		{
			...
			len = recv(socket,pdata,rx_len);
			printf("\r\n(wzfu print test)接收到的数据长度 %d\r\n", len);
		}
    }while((len == 0)&&((get_sys_tick()-timeout_tick)<Timeout));
	*RcvDatalen = len;
	if(len > 0)
	{
	    ret = MY_TLS_STATUS_OK; 
	}
	
	return ret;
}
MY_TLS_Status_t SendData(uint8_t socket, uint8_t *pdata, uint16_t Reqlen, uint16_t *SentDatalen, uint32_t Timeout)
{
	MY_TLS_Status_t ret = MY_TLS_STATUS_ERROR;
	uint64_t timeout_tick;
...
int mbedtls_net_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    do {
        if (len > MY_TLS_PAYLOAD_SIZE(fd)) {
            printf("net_send: buffer length = %d, split data sending\r\n", len);
            once_len = MY_TLS_PAYLOAD_SIZE(fd);
            len -= MY_TLS_PAYLOAD_SIZE(fd);
        } else {
            once_len = len;
            len = 0;
        }

        ret = SendData((uint8_t)fd,
                            pdata, once_len,
                            &send_size, 200);
        if (ret != MY_TLS_STATUS_OK) {
...
{//mbedtls_net_recv_timeout函数实现mbedtls_net_recv函数就可以不用实现,mbedtls_net_recv_timeout为NULL时才调用mbedtls_net_recv
    MY_TLS_Status_t ret;
    uint16_t recv_size;
    int fd = ((mbedtls_net_context *) ctx)->fd;
    ...
    } while (ret == MY_TLS_STATUS_OK && recv_size == 0);
    return recv_size;
}
/**
...
int mbedtls_net_connect(mbedtls_net_context *ctx, const char *host, const char *port, int proto)
{
    MY_TLS_Status_t ret;
    uint8_t type;
//    uint8_t ip_addr[20]; /*This array can be used as string or integer array*/
    int     trials=1;
    int socket = 0;
	uint8 s_ip[4] = {139,196,135,135};
	uint16_t l_port = 6000;
//	static uint16_t l_port_cnt = 0;
	//extern uint8_t server_ip[4];//mqtt服务器的ip地址
//	l_port = l_port+l_port_cnt;
//	l_port_cnt++;
#if 0    
//    do
//    {
...
//		if (ret != MY_TLS_STATUS_OK) {
//		    printf("net_connect: get host addr fail - %d , trial : %d/5 \r\n", ret , trials);
//		}
//		trials++;
//    }while(ret != MY_TLS_STATUS_OK);
#else
    //while(do_dns((uint8_t *)host,s_ip)==0);
#endif
    printf("%d.%d.%d.%d\r\n",s_ip[0],s_ip[1],s_ip[2],s_ip[3]);    
	printf("%s,port:%d,l_port:%d",host,atoi(port),l_port);
    type = proto == MBEDTLS_NET_PROTO_UDP ?
                    Sn_MR_UDP : Sn_MR_TCP;
    ret = OpenClientConnection(socket, type, "", s_ip, atoi(port), l_port);
    if (ret != MY_TLS_STATUS_OK) {
