...
{
    int res = 0;
    const char *fmt = "/%s/%s/user/led";//const char *fmt = "/%s/%s/user/get";
    char *topic = NULL;
    int topic_len = 0;
...
//    HAL_GetDeviceSecret(g_device_secret);
     ...
     */
	 //can add
     mqtt_params.host = "a1TYwrkFQNN.iot-as-mqtt.cn-shanghai.aliyuncs.com"; 
...
//        }
        ...
        IOT_MQTT_Yield(pclient, 10);//IOT_MQTT_Yield(pclient, 200);
