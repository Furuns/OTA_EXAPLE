...
//extern uint8_t tick_mutual_flag;
//extern uint8_t mqtt_connedted;
...
int HAL_GetDeviceName(char device_name[IOTX_DEVICE_NAME_LEN + 1])
{
	char *name;
	name = device_name;
    int len = strlen(g_device_name);
    if(len >= IOTX_DEVICE_NAME_LEN){
        return -1;
    }
    memset(name, 0x0, IOTX_DEVICE_NAME_LEN); 
    strncpy(name, g_device_name, len); 
...
int HAL_GetDeviceSecret(char device_secret[IOTX_DEVICE_SECRET_LEN + 1])
{
    ...
    int len = strlen(g_device_secret); 
    if(len >= IOTX_DEVICE_SECRET_LEN){
        return -1;
    }
    
    memset(secret, 0x0, IOTX_DEVICE_SECRET_LEN); 
    strncpy(secret, g_device_secret, len); 
    return len;
}
...
int HAL_GetProductKey(char product_key[IOTX_PRODUCT_KEY_LEN + 1])
{
	char *key;
	key = product_key;
    int len = strlen(g_product_key); 
    if(len >= IOTX_PRODUCT_KEY_LEN){
        return -1;
    }
    memset(key, 0x0, IOTX_PRODUCT_KEY_LEN); 
    strncpy(key, g_product_key, len); 
    return len;
}
...
{
    return malloc(size);//return mymalloc(SRAMIN,size);
}
...
void HAL_SleepMs(uint32_t ms)
{
	...
	timeout_tick = HAL_UptimeMs();
	while ((HAL_UptimeMs()-timeout_tick) < ms);
}
...
{
//	HAL_Printf("ssl destroy,handle: %d\n",handle);
//	close((SOCKET)handle);
    my_HAL_SSL_Destroy(handle);
	return 0;
}
...
{
	return my_HAL_SSL_Establish(host,port,ca_crt,ca_crt_len);
}
...
{
	return my_HAL_SSL_Read(handle,buf,len,timeout_ms);
}
...
{
	return my_HAL_SSL_Write(handle,buf,len,timeout_ms);
}
...
int HAL_GetProductSecret(char product_secret[IOTX_PRODUCT_SECRET_LEN + 1])
{
    ...
    int len = strlen(g_product_secret);
    memset(secret, 0x0, IOTX_PRODUCT_SECRET_LEN + 1);

    strncpy(secret, g_product_secret, len);
